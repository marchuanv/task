...
const suite = describe('when queueing a short running tasks given a repeat no data resolve', () => {
    it('should run indefinitely', async () => {
        const promises = [
            TestTask.create(suite, 'RepeatNoDataResolveShortTaskA', [TaskFlag.RepeatNoOutput]).run(0, async function () { }),
            TestTask.create(suite, 'RepeatNoDataResolveShortTaskB', [TaskFlag.RepeatNoOutput]).run(0, async function () { }),
            TestTask.create(suite, 'RepeatNoDataResolveShortTaskC', [TaskFlag.RepeatNoOutput]).run(0, async function () { })
        ];
