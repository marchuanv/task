...
const suite = describe('when queueing a long running tasks given a repeat no data resolve', () => {
    it('should run indefinitely', async () => {
        const promises = [
            TestTask.create(suite, 'RepeatNoDataResolveLongTaskA', [TaskFlag.RepeatNoOutput]).run(0, async function () {
                await this.simulateDelay();
            }),
            TestTask.create(suite, 'RepeatNoDataResolveLongTaskB', [TaskFlag.RepeatNoOutput]).run(0, async function () {
                await this.simulateDelay();
            }),
            TestTask.create(suite, 'RepeatNoDataResolveLongTaskC', [TaskFlag.RepeatNoOutput]).run(0, async function () {
                await this.simulateDelay();
            })
        ];
