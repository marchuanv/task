...
const suite = describe('when queueing a long running tasks given a once off no data resolve', () => {
    it('should run once', async () => {
        const promises = [
            TestTask.create(suite, 'OnceOffNoDataResolveLongTaskA', [TaskFlag.OnceOffNoOutput]).run(0, async function () {
                await this.simulateDelay();
            }),
            TestTask.create(suite, 'OnceOffNoDataResolveLongTaskB', [TaskFlag.OnceOffNoOutput]).run(0, async function () {
                await this.simulateDelay();
            }),
            TestTask.create(suite, 'OnceOffNoDataResolveLongTaskC', [TaskFlag.OnceOffNoOutput]).run(0, async function () {
                await this.simulateDelay();
            })
        ];
