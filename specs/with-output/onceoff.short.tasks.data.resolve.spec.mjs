...
const suite = describe('when queueing a short running tasks given a once off data resolve', () => {
    it('should run once', async () => {
        const promises = [
            TestTask.create(suite, 'OnceOffDataResolveShortTaskA', [TaskFlag.OnceOffWithOutput]).run(0, async function () {
                this.complete('OnceOffDataResolveShortTaskASuccess');
            }),
            TestTask.create(suite, 'OnceOffDataResolveShortTaskB', [TaskFlag.OnceOffWithOutput]).run(0, async function () {
                this.complete('OnceOffDataResolveShortTaskBSuccess');
            }),
            TestTask.create(suite, 'OnceOffDataResolveShortTaskC', [TaskFlag.OnceOffWithOutput]).run(0, async function () {
