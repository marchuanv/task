...
const suite = describe('when queueing a short running tasks given a once off no data resolve', () => {
    it('should run once', async () => {
        const promises = [
            TestTask.create(suite, 'OnceOffNoDataResolveShortTaskA', [TaskFlag.OnceOffNoDataResolve]).run(0, async function () { }),
            TestTask.create(suite, 'OnceOffNoDataResolveShortTaskB', [TaskFlag.OnceOffNoDataResolve]).run(0, async function () { }),
            TestTask.create(suite, 'OnceOffNoDataResolveShortTaskC', [TaskFlag.OnceOffNoDataResolve]).run(0, async function () { })
        ];
