...
const suite = describe('when queueing a long running tasks given a repeat data resolve', () => {
    it('should run indefinitely', async () => {
        const promises = [
            TestTask.create(suite, 'RepeatDataResolveLongTaskA', [TaskFlag.RepeatDataResolve]).run(0, async function () {
                await this.simulateDelay();
                this.complete('RepeatDataResolveLongTaskASuccess');
            }),
            TestTask.create(suite, 'RepeatDataResolveLongTaskB', [TaskFlag.RepeatDataResolve]).run(0, async function () {
                await this.simulateDelay();
                this.complete('RepeatDataResolveLongTaskBSuccess');
            }),
            TestTask.create(suite, 'RepeatDataResolveLongTaskC', [TaskFlag.RepeatDataResolve]).run(0, async function () {
                await this.simulateDelay();
