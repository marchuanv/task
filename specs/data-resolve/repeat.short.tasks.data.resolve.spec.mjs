...
const suite = describe('when queueing a short running tasks given a repeat data resolve', () => {
    it('should run indefinitely', async () => {
        const promises = [
            TestTask.create(suite, 'RepeatDataResolveShortTaskA', [TaskFlag.RepeatDataResolve]).run(0, async function () {
                this.complete('RepeatDataResolveShortTaskASuccess');
            }),
            TestTask.create(suite, 'RepeatDataResolveShortTaskB', [TaskFlag.RepeatDataResolve]).run(0, async function () {
                this.complete('RepeatDataResolveShortTaskBSuccess');
            }),
            TestTask.create(suite, 'RepeatDataResolveShortTaskC', [TaskFlag.RepeatDataResolve]).run(0, async function () {
