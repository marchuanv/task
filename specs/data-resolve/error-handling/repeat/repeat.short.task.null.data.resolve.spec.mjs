...
const suite = describe('when a repeated short task is resolved with null given an error', () => {

    let promise;
    beforeAll(async () => {
        promise = TestTask.create(suite, 'ShortTaskRepeatedDataResolvedWithNull', [TaskFlag.RepeatDataResolve]).run(0, async function () {
            this.complete(null);
        });
    });
