...
const suite = describe('when a repeated short task is resolved with null given error handling', () => {

    let promise;
    beforeAll(async () => {
        promise = TestTask.create(suite, 'ShortTaskRepeatedDataResolvedWithNull', [TaskFlag.RepeatDataResolve]).queue(0, async function () {
            this.complete(null);
        });
    });
