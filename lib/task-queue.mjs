    ...
    setImmediate(async () => {
        ...
        if (queue.length > 0) {
            ...
            if (privateBag.get(TaskQueue).length > 0) {
                for (const taskWithError of privateBag.get(TaskQueue)) {
                    const { error } = privateBag.get(taskWithError);
                    TaskLogging.log(taskWithError, `State: ${taskWithError.state.toString()}`);
                    TaskLogging.log(taskWithError, error);
                }
            }
            TaskLogging.log({ Id: 'END', flags: [] }, `no more tasks in the queues`);
        }
    });
}
