            ...
            try {
                ...
                const handleState = () => {
                    ...
                    if (diff > longRunningTimeoutMilli) {
                        ...
                        properties.state = TaskState.Error;
                        properties.states.push(properties.state);
                    } else if (properties.state !== TaskState.LongRunning) {
                        ...
                        if (diff > longRunningMilli && canSetToLongRunning) {
                    ...
                    switch (task.state) {
                        ...
                        case TaskState.Error: {
                            if (task.hasFlag(TaskFlag.HandleErrors)) {
                                ...
                                privateBag.get(TaskQueue).push(task);
                                return taskLoop(100); //current task is in error, but config with don't halt on this error, carry on with other tasks...
                            } else {
