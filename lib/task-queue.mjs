                        ...
                        switch (task.state) {
                            ...
                            case TaskState.PromiseResolvedNoResults: {
                                if (task.hasFlag(TaskFlag.RepeatNoDataResolve)) {
                                    ...
                                    properties.error = new Error(`${task.toString()} task is configured to repeat and resolve with data, but was not resolved with valid data`);
                                    properties.state = TaskState.Error;
                                    properties.states.push(properties.state);
                                    handleState();
                                    break;
                                } else if (task.hasFlag(TaskFlag.OnceOffDataResolve)) {
                                    properties.error = new Error(`${task.toString()} task is configured to be once off and resolve with data, but was not resolved with valid data`);
                                    properties.state = TaskState.Error;
                                    properties.states.push(properties.state);
                                    handleState();
                                    break;
                            ...
                            case TaskState.Error: {
                                if (task.hasFlag(TaskFlag.HandleErrors)) {
                                    ...
                                    properties.reject(properties.error);
...
export class TaskQueue {
    ...
    */
    static enqueue(task) {
        setTimeout(() => {
            const properties = privateBag.get(task);
            const { state, name, contextId, dependencies, callback, resolve, reject } = properties;
            if (!callback || !resolve || !reject) {
