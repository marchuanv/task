                    ...
                    const handleState = () => {
                        ...
                        switch (task.state) {
                            case TaskState.Requeue: {
                                TaskQueue.enqueue(task, properties);
                                break;
                            }
                            ...
                            case TaskState.PromiseResolvedWithResults: {
                                if (task.hasFlag(TaskFlag.RepeatNoDataResolve)) {
                                    properties.error = new Error(`${task.toString()} task is configured to NOT resolve with data`);
                                    properties.state = TaskState.Error;
                                    properties.states.push(properties.state);
                                    handleState();
                                    break;
                            ...
                            case TaskState.PromiseResolvedNoResults: {
                                if (task.hasFlag(TaskFlag.RepeatNoDataResolve)) {
                                    ...
                                    properties.error = new Error(`${task.toString()} task is configured to resolve with data, but was not resolved with valid data`);
                                    properties.state = TaskState.Error;
                                    properties.states.push(properties.state);
                                    handleState();
                                    break;
...
export class TaskQueue {
    ...
    */
    static enqueue(task, properties) {
        setTimeout(() => {
            const { state, name, contextId, dependencies, callback, reject } = properties;
            if (!callback) {
                properties.error = new Error(`${task.toString()} was not queued, no callback function`);
                return reject(properties.error);
            }
